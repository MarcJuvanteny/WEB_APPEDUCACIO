      ...
      <RevealGroup className="mt-14 grid grid-cols-1 gap-5 md:grid-cols-3">
        {t.features.items.map((item, index) => {
          ...
          return (
            <RevealItem
              key={index}
              className={`${SPANS[index]} rounded-card border border-ink/10 bg-surface p-8 transition-[transform,border-color] duration-200 ease-[var(--ease-out-strong)] hover:-translate-y-1 hover:border-terracotta/25`}
            >
